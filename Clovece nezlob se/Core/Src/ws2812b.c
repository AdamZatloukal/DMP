...
uint8_t LED_data[NUM_OF_LEDS][3];				// Stores RGB decimal RGB values for each LED and the index of the LED
...
// Puts the data into correct position and each bit gets assigned DCL which is then sent using PWM on timer 1
void send_data(void){
	uint32_t color_bits;
	int LED_index = 0;
	int i;
	...
	for(i =0; i < NUM_OF_LEDS; i++){
		...
		for(j = 23; j >= 0; j--){														// Iterates 24 times to go thorough each bit of color_bits
			if(color_bits & (1 << j)){													// Creates a 24b mask where only jth bit is 1 and compares it to color_bits
